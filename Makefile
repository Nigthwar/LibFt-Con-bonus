# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    Makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: cguerrei <cguerrei@student.42malaga.com>   +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2023/04/19 13:42:44 by cguerrei          #+#    #+#              #
#    Updated: 2023/05/02 17:43:17 by cguerrei         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

NAME = libft.a

LIB = ar rcs

SRC = ft_atoi.c ft_isalnum.c ft_isalpha.c \
 ft_isprint.c ft_strlen.c ft_tolower.c \
 ft_strlcat.c ft_strncmp.c ft_strncpy.c ft_isdigit.c\
 ft_isascii.c ft_strlcpy.c ft_memset.c ft_memcpy.c\
 ft_bzero.c ft_memmove.c ft_toupper.c ft_strchr.c\
 ft_strrchr.c ft_memchr.c ft_strnstr.c ft_memcmp.c\
 ft_calloc.c ft_strdup.c ft_substr.c ft_strjoin.c\
 ft_strjoin.c ft_strtrim.c ft_itoa.c ft_strmapi.c\
 ft_striteri.c ft_putchar_fd.c ft_putstr_fd.c \
 ft_putendl_fd.c ft_putnbr_fd.c ft_split.c

SRC_BONUS = ft_lstnew.c ft_lstadd_front.c\
ft_lstsize.c ft_lstlast.c ft_lstadd_back.c\
ft_lstdelone.c ft_lstclear.c ft_lstiter.c\
ft_lstmap.c
 
OBJ = $(SRC:.c=.o)

OBJ_BONUS = $(SRC_BONUS:.c=.o)

RM = rm -f


CC = gcc

CFLAGS = -Wall -Werror -Wextra

all: $(NAME)

$(NAME): $(OBJ)
		$(LIB) $(NAME) $(OBJ)


bonus: $(OBJ_BONUS) $(OBJ)
		$(LIB) $(NAME) $(OBJ) $(OBJ_BONUS)

clean: 
	$(RM) $(OBJ) $(OBJ_BONUS)

fclean: clean
		$(RM) $(NAME)

re: fclean all

%.o: %.c
	$(CC) $(CFLAGS) -c -o $@ $<
comp:
		$(CC) $(CFLAGS) ft_strnstr.c ft_strlen

.PHONY: all clean fclean re bonus comp



